...
// This test calls Resend's live API, so it can only run where credentials exist. Skipping when the
// key is absent keeps `pnpm test` meaningful locally and in CI instead of reporting a failure that
